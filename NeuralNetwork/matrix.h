...
class matrix {
   private:
      std::vector<T> m_elements; // array of elements
   public:
      const unsigned m_rows;  // number of rows
      const unsigned m_cols;  // number of columns
...
matrix<T>::matrix( const matrix<T>& cp )
  : m_rows(cp.rows), m_cols(cp.cols), m_elements(cp.elements)
{
}
